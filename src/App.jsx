      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          ...
          <div className="glass-card p-6 md:p-8 rounded-[2.5rem] flex flex-col md:flex-row gap-4 items-center border border-blue-500/10 shadow-lg">
            ...
            <button disabled={loading} onClick={submitAbsenClick} className={`w-full md:w-auto px-10 py-5 bg-blue-600 text-white rounded-2xl font-black uppercase text-[10px] btn-3d shadow-xl shadow-blue-600/30 transition-all active:scale-95 ${(!photo || !isSynced) ? 'opacity-70' : 'opacity-100'}`}>
...
function KelolaView({ currentUser, pegawaiList, passRequests, locationConfig, showToast, db, appId, credentials }) {
  ...
  const exportPegawaiToExcel = async () => {
    ...
    const exportData = pegawaiList.map((p, idx) => ({
      "No": idx + 1,
      "NIP": p.nip,
      "Nama Pegawai": p.nama,
      "Bidang": p.bidang,
      "Jabatan": p.jabatan,
      "Hak Akses Admin": p.isAdmin ? "Ya" : "Tidak",
      "Bypass GPS": p.bypassGps ? "Ya" : "Tidak"
    }));
